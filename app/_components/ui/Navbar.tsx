...
export const Navbar: React.FC<NavbarProps> = ({ selected }) => {
  ...
  return (
    <nav className="flex w-screen justify-between text-black bg-white border-t border-gray-300 px-8 py-4 gap-[10px] sticky bottom-0">
      <NavbarButton
        text="Reabastercer"
      ...
      <NavbarButton
        text="Perfil"
        selected={selectedButton === 'Perfil'}
...
export const NavbarButton: React.FC<NavBarButtonProps> = ({ text, href = '/', selected, icon, onClick }) => {
  return (
    <Link
      href={href}
      onClick={onClick}
    ...
    >
      <div className="relative aspect-square h-6">{icon}</div>
      {text}
    </Link>
  );
};
